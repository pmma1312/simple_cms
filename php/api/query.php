...
if(!isset($_GET['no_limit'])) {
  ...
  $query = "SELECT content.cid, admin.username, content.title, content.content, DATE_FORMAT(content.entry_date, '%d.%m.%Y') AS entry_date FROM content JOIN admin ON content.aid = admin.aid WHERE content.deleted != 1 ORDER BY content.cid DESC";
}
